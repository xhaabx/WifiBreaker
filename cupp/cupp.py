...
def print_to_file(filename, unique_list_finished):
	...
	for line in f:
		lines += 1
	f.close()
	print "[+] Salvando Dicionario \033[1;31m"+filename+"\033[1;m, com \033[1;31m"+str(lines)+" palavras.\033[1;m"
...
if len(sys.argv) < 2 or sys.argv[1] == '-h':
	...
	print "\r\n[+] Insira as informacoes sobre o alvo para criar o dicionario"
	print "[+] Se voce nao sabe a informacao, basta pressionar enter! ;)\r\n"

# We need some informations first!

	name = raw_input("> Primeiro Nome: ").lower()
	while len(name) == 0 or name == " " or name == "  " or name == "   ":
		print "\r\n[-] Voce deve inserir o nome!"
	...
	while len(birthdate) != 0 and len(birthdate) != 8:
		print "\r\n[-] Voce deve inserir 8 numeros para o aniversario!"
		birthdate = raw_input("> Aniversario (DDMMYYYY): ")
	birthdate = str(birthdate)

	print "\r\n"

	wife = raw_input("> Nome do parceiro(a): ").lower()
	wifen = raw_input("> Apelido do parceiro(a): ").lower()
	wifeb = raw_input("> Aniversario do parceiro(a) (DDMMYYYY): ")
	...
	kidn = raw_input("> Apelido do filho(a): ").lower()
	kidb = raw_input("> Aniversario do filho(a) (DDMMYYYY): ")
	while len(kidb) != 0 and len(kidb) != 8:
		print "\r\n[-] Voce deve inserir 8 digitos para o aniversario!"
		kidb = raw_input("> Aniversario do filho (DDMMYYYY): ")
	kidb = str(kidb)
	print "\r\n"

	pet = raw_input("> Nome do animal de estimacao: ").lower()
	company = raw_input("> Nome da empresa atual: ").lower()
	print "\r\n"

	words = ['']
	words1 = raw_input("> Voce quer adicionar algumas palavras chaves sobre o alvo? Y/[N]: ").lower()
	words2 = ""
	if words1 == "y":
		words2 = raw_input("> Por favor insira as palavras separadas por virgulas. [i.e. hacker,juice,black], espacos serao removidos: ").replace(" ","")
	words = words2.split(",")

	spechars = ['']
	spechars1 = raw_input("> Voce quer adicionar caracteres especiais ao final das palavras? Y/[N]: ").lower()
	if spechars1 == "y":
		for spec1 in chars:
			spechars.append(spec1)
			for spec2 in chars:
	...
	randnum = raw_input("> Voce quer adicionar alguns numeros aleatorios ao final das palavras? Y/[N]:").lower()
	leetmode = raw_input("> Leet mode? (ex: Gabriel = G@BR13L) Y/[N]: ").lower()


	print "\r\n[+] Agora criando o dicionario..."
...
# Birthdays first
	...
	print "[+] Organizando a lista e removendo duplicatas..."
	...
	print_to_file('Dic_GSI'+'.txt', unique_list_finished)
	exit()
